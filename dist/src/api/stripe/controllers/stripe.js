...
exports.default = {
    ...
    async createPaymentIntent(ctx) {
        try {
            ...
            const result = await strapi.service('api::stripe.stripe').createPaymentIntent(amount, currency, metadata);
            if (result.success) {
                return ctx.send({
                    success: true,
                    data: result.data,
    ...
    async createCheckoutSession(ctx) {
        try {
            ...
            const result = await strapi.service('api::stripe.stripe').createCheckoutSession(amount, currency, metadata);
            if (result.success) {
                return ctx.send({
                    success: true,
                    data: result.data,
    ...
    async webhook(ctx) {
        try {
            ...
            const validation = strapi.service('api::stripe.stripe').validateWebhook(payload, signature);
